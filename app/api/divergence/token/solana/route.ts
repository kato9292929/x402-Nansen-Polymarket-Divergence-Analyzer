...
async function fetchNansenTokenData(token: string) {
  ...
  if (!apiKey) return getMockNansenData(token);
  try {
    ...
    console.error("fetchNansenTokenData (solana) error:", e);
...
async function fetchPolymarketTokenData(token: string) {
  try {
    ...
    console.error("fetchPolymarketTokenData (solana) error:", e);
...
JSON形式で出力:
{
  ...
  "dataPoints": {
    ...
    console.error("/api/divergence/token/solana error:", e);
...
const x402Handler = withX402(
  handler,
  {
    accepts: { scheme: "exact", price: "$0.30", network: SOLANA_NETWORK, payTo: SOLANA_PAY_TO },
